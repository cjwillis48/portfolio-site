<script lang="ts">
	...
	import { formatExperienceDate } from '$lib/utils/experience';
</script>
...
<div class="max-w-4xl mx-auto px-4 py-16 sm:py-24">
	...
	<p class="text-lg text-slate-500 mb-16 dark:text-slate-400">
	...
	<section class="mb-20">
		...
		<div class="flex flex-col gap-0">
			{#each experience as job, i}
				<div
					class="relative pl-8 pb-10 {i < experience.length - 1
						? 'border-l-2 border-slate-200 dark:border-slate-700'
						: ''}"
				>
					<!-- Dot -->
					<div
						class="absolute left-0 top-1 w-4 h-4 rounded-full bg-indigo-600 border-2 border-white ring-2 ring-indigo-200 -translate-x-[9px] dark:border-slate-950 dark:ring-indigo-400/40"
					></div>
					...
					>
						<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1 mb-1">
							...
							<span class="text-sm text-slate-400 shrink-0 dark:text-slate-500"
								>{formatExperienceDate(job.start)} – {formatExperienceDate(job.end)}</span
							>
						</div>
						<p class="text-indigo-600 font-medium text-sm mb-4 dark:text-indigo-300">
							{job.company}
						</p>
	...
	<section>
		...
		<div class="grid sm:grid-cols-2 gap-6">
			{#each skills as group}
				...
				>
					<h3
						class="font-semibold text-slate-900 text-sm uppercase tracking-wide mb-4 dark:text-slate-100"
					>
						{group.category}
					</h3>
					<div class="flex flex-wrap gap-2">
